	...
	{	
		...
		if (par4 >= 1 && par4 + l + 1 <= 256)
		{
				...
				for (int l1 = par3 - b0; l1 <= par3 + b0 && flag; ++l1)
				{
					for (j1 = par5 - b0; j1 <= par5 + b0 && flag; ++j1)
					{
						if (i1 >= 0 && i1 < 256)
						{
							...
							Block block = BashPackCore.instance.blockRawBacon;
							flag = true;
						}
						else
						{
			...
			else
			{
				...
				/** Custom grass block **/
				Block soil = BashPackCore.instance.blockRawBacon;
				...
				if (isSoil && par4 < 256 - l - 1)
				{
						...
						for (j2 = par3 - i2; j2 <= par3 + i2; ++j2)
						{
							...
							for (int l2 = par5 - i2; l2 <= par5 + i2; ++l2)
							{
								...
								if (Math.abs(k2) != i2 || Math.abs(i3) != i2 || par2Random.nextInt(2) != 0 && k1 != 0)
								{
									...
									{
										/** Custom leaf block **/
										this.setBlockAndMetadata(par1World, j2, j1, l2, Block.melon.blockID, 0);
									}
								}
							}
						}
					...
					for (j1 = 0; j1 < l; ++j1)
					{
						...
						{
							/** Custom Log block **/
							this.setBlockAndMetadata(par1World, par3, par4 + j1, par5, BashPackCore.instance.blockBurntBacon.blockID, 0);
						}
					}
